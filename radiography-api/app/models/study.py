...
# ── NEW: serve the actual DICOM bytes ────────────────────────────────────
#
# Cornerstone's WADO URI loader fetches each image as a plain HTTP GET.
# Previously there was no route for this — every image request returned 404
# and Cornerstone failed silently, leaving the viewer blank.
#
# The viewer builds its URL as:
#   wadouri:/api/v1/studies/{studyId}/dicom/{filename}
# which maps directly to this endpoint.
...
async def serve_dicom_file(
    ...
    # Path-traversal guard — filename must be a plain name, no slashes or dots
    if not filename or "/" in filename or "\\" in filename or filename.startswith(".."):
        raise HTTPException(status_code=400, detail="Invalid filename")

    file_path = os.path.join(study.dicom_path, filename)

    # Double-check the resolved path stays inside the study folder
    ...
    return FileResponse(
        ...
        # Let browsers / CDNs cache individual DICOM slices for 1 h —
        # they're immutable once uploaded.
...
#
# Previous version had two critical performance bugs:
#   1. content = await file.read()  →  entire ZIP loaded into RAM before
#      anything else happens. A 100-slice CT ZIP (~50 MB) would consume
#      50 MB of RAM per concurrent upload and block the event loop.
#   2. _extract_archive() and _ingest_extracted_files() are synchronous
#      (zipfile, shutil, os.walk) — they ran directly in the async handler,
#      blocking the whole uvicorn worker for the duration of extraction.
#
# Fixes:
#   • Stream the incoming file to disk in 1 MB chunks — memory stays flat.
#   • Run both sync functions in the default threadpool via run_in_executor
...
async def upload_dicom(
    ...
    if ext in ARCHIVE_EXTENSIONS:
        # ── Stream ZIP/RAR to a temp file, then extract in a thread ──────────
        with tempfile.TemporaryDirectory() as tmp:
            archive_path = os.path.join(tmp, original_name)

            # Chunked write — keeps RAM usage constant regardless of file size
            ...
            # Both extraction and ingestion are blocking I/O — run in threadpool
        ...
        # Images are small — read once then convert in threadpool
        ...
        # Single DICOM file — stream to disk, then verify header
        ...
        # Verify it's actually DICOM (header only — avoids re-reading pixel data)
